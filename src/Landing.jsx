...
export default function FindeLanding() {
  ...
  const submitIframeRef = useRef(null);
  ...
  useEffect(() => () => {
    ...
    if (submitIframeRef.current?.parentNode) {
      submitIframeRef.current.parentNode.removeChild(submitIframeRef.current);
    }
  }, []);
  ...
  const handleSubmit = () => {
    ...
    submitIframeRef.current = iframe;

    submitTimerRef.current = setTimeout(() => {
      if (iframe.parentNode) document.body.removeChild(iframe);
      submitIframeRef.current = null;
      ...
      <div className="landing">
        ...
        <nav className={`nav ${scrolled ? "nav-scrolled" : ""}`}>
          <div className="nav-inner">
            ...
            <button type="button" className="nav-cta" onClick={() => document.getElementById("registro")?.scrollIntoView({ behavior: "smooth", block: "center" })}>
              Pre-registrarme
            </button>
          </div>
        </nav>
        ...
        <section className="value">
          <div className="section-inner">
            <h2 className="section-title">
              Pensado para el viajero peruano<span className="dot-accent">.</span>
            </h2>
...
@media (max-width: 768px) { .landing section[id], .landing [id="registro"] { scroll-margin-top: 70px; } }
...
/* NAV */
.nav { position: fixed; top: 0; left: 0; right: 0; z-index: 100; padding: 18px 40px; transition: padding .25s ease, background-color .25s ease, border-color .25s ease, box-shadow .25s ease; }
.nav-scrolled { background: rgba(250, 250, 247, .92); -webkit-backdrop-filter: blur(20px); backdrop-filter: blur(20px); border-bottom: 1px solid rgba(0,0,0,.05); padding: 12px 40px; box-shadow: 0 4px 12px rgba(27,58,45,.04); }
...
.hero-dest { aspect-ratio: 3/2; border-radius: 14px; display: flex; align-items: flex-end; padding: 14px; transition: transform .3s ease, box-shadow .3s ease; cursor: default; position: relative; overflow: hidden; background-size: cover; background-position: center; }
.hero-dest:hover { transform: translateY(-4px); box-shadow: 0 12px 24px rgba(27,58,45,.18); }
.hero-dest:hover .hero-dest-img { transform: scale(1.06); }
...
.consent { display: flex; gap: 10px; align-items: flex-start; font-size: 12.5px; color: var(--gy-soft); line-height: 1.5; cursor: pointer; padding-top: 2px; -webkit-user-select: none; user-select: none; }
.consent input[type="checkbox"] { width: 18px; height: 18px; margin-top: 0; accent-color: var(--f); cursor: pointer; flex-shrink: 0; }
.consent input[type="checkbox"]:focus-visible { outline: 2px solid var(--tr); outline-offset: 2px; }
...
.btn-primary:disabled { opacity: .45; cursor: not-allowed; background: var(--gy); box-shadow: none; }
.btn-primary:disabled:hover { transform: none; box-shadow: none; background: var(--gy); }
.btn-full { width: 100%; }
...
/* VALUE */
.value { padding: 120px 40px; }
.value .section-title { text-align: center; margin-bottom: 48px; }
.benefits { display: grid; grid-template-columns: repeat(3, 1fr); gap: 28px; max-width: 1000px; margin: 0 auto; }
.benefit { background: white; border: 1px solid rgba(0,0,0,.05); border-radius: 18px; padding: 36px; min-height: 220px; transition: transform .3s ease, box-shadow .3s ease, border-color .3s ease; }
.benefit:hover { transform: translateY(-4px); box-shadow: 0 12px 32px rgba(27,58,45,.08); border-color: rgba(199,97,58,.25); }
.benefit-icon-wrap { width: 48px; height: 48px; border-radius: 12px; background: rgba(27,58,45,.06); display: flex; align-items: center; justify-content: center; color: var(--f); margin-bottom: 20px; }
.benefit-icon { width: 22px; height: 22px; }
.benefit-title { font-weight: 700; font-size: 16px; margin-bottom: 12px; color: var(--ch); }
.benefit-desc { font-size: 15px; color: var(--gy); line-height: 1.7; }
...
.faq-item summary { padding: 18px 24px; min-height: 56px; font-weight: 700; font-size: 15px; cursor: pointer; list-style: none; color: var(--ch); display: flex; justify-content: space-between; align-items: center; gap: 16px; transition: color .2s; }
...
.team-pillar { background: white; border: 1px solid rgba(0,0,0,.05); border-radius: 16px; padding: 28px; transition: transform .3s ease, box-shadow .3s ease; }
.team-pillar:hover { transform: translateY(-3px); box-shadow: 0 10px 24px rgba(27,58,45,.06); }
...
@media (max-width: 1024px) {
  ...
  .hero-grid { grid-template-columns: 1fr; gap: 48px; max-width: 560px; }
  ...
  .benefits { grid-template-columns: 1fr; max-width: 520px; margin: 0 auto; gap: 20px; }
...
@media (max-width: 640px) {
  ...
  .hero-destinations { grid-template-columns: repeat(2, 1fr); gap: 14px; }
  .hero-dest-name { font-size: 15px; }
  .problem { padding: 56px 20px; }
  .problem-stats { grid-template-columns: 1fr; gap: 28px; }
  .problem-divider { display: none; }
  .value, .mockup, .faq, .team { padding: 60px 20px; }
  .value { padding: 64px 16px; }
  .value .section-title { margin-bottom: 32px; }
  ...
  .benefits { gap: 16px; }
  .team-pillar { padding: 24px; }
  .faq-item summary { padding: 16px 20px; font-size: 14px; }
  .faq-item p { padding: 0 20px 18px; font-size: 13.5px; }
...
@media (max-width: 380px) {
  ...
  .hero-destinations { gap: 12px; }
  .problem { padding: 48px 16px; }
  .value, .mockup, .faq, .team { padding: 56px 16px; }
