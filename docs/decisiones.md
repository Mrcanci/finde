...
## La celda escasa del calendario no lleva punto de color

*2026-08-18*

**Decisión:** el aviso de "quedan pocos cupos" sale del texto de 8px que vivía
dentro de la celda y se reparte en tres lugares: el **tinte terracota de la
celda**, que ya existía, una **leyenda dentro de la tarjeta del calendario** que
nombra las fechas, y el **`aria-label` del botón**.

**Descartado:** el punto de color en la celda, que era la opción anotada en
`docs/estado.md` desde antes de medirla.

**Razón, y sale de mirar las dos variantes en pantalla, no de discutirlas.** Se
montaron las dos a 390 y 412, con agosto a la vista, en los dos estados que
importan:

1. **Sin elegir, el punto no aporta.** La celda escasa ya lleva tres señales
   propias: tinte terracota al 18%, borde terracota de 1,5px y el número en
   terracota. El punto es la cuarta sobre la misma celda de 42px.
2. **Con la fecha ya elegida, el punto no se ve.** Selección y escasez
   superpuestas llenan la celda de terracota sólido, y un punto terracota sobre
   terracota sólido desaparece. O sea que **falla justo en el estado donde más
   importaría**, que es cuando el viajero ya eligió la fecha con un cupo.
3. Sostenerlo obliga a **invertirlo a blanco**: una regla condicional de color
   para mantener un elemento que en el estado normal ya era redundante.

**El `aria-label` no es un extra, es lo que evita una regresión.** Hasta hoy el
lector de pantalla leía "23 Último cupo" sin que nadie hubiera escrito un label:
lo cargaba el texto que estaba dentro del botón. Sacar ese texto sin poner el
label deja el botón en "23" pelado y **borra la escasez para quien no la ve**,
sin que se note en la pantalla. Queda escrito en el código, al lado del atributo.

**Consecuencia en el código:** `MonthCalendar` (`src/AppDemo.jsx`) recorre el mes
una sola vez y de esa pasada salen las celdas y la leyenda, con `isLowSeats` y
`lowSeatsText` como única definición de la escasez, para que la leyenda no pueda
nombrar una fecha que la grilla no pintó. El porqué del "sin punto" está
comentado en el propio render, con la medición, porque es de las cosas que
alguien vuelve a proponer como mejora obvia.

**Verificación:** volcado del rectángulo de cada celda antes y después, a 390 y
1440, sobre los dos calendarios (el de reservar y el del paso 3 del formulario de
tour): **198 celdas comparadas, 0 movidas.** Lo único que cambia de tamaño es la
tarjeta del calendario con escasez, que crece 28,19px, que es exactamente la
leyenda nueva. El calendario del formulario de tour no cambia ni un píxel, porque
nunca recibe `lowDates`.
