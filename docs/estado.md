...
- [x] **Recuperar contraseña: IMPLEMENTADO el 2026-08-19.** El enlace "¿Olvidaste tu contraseña?" vive dentro de `AuthForm`, así que sale en el login y en el modal del checkout con un solo cambio, y **como tercer modo del formulario, no como ruta**: en el modal navegar sería romperlo. La pantalla nueva es `/nueva-contrasena`. Va con el **borrador del checkout persistido** (tour, ruta, fecha, cupos y paso; los datos personales NO), que además es la precondición ya anotada para "entrar con Google". El hallazgo que ordenó la tanda, y que está comentado en el código: **el enlace del correo ES un inicio de sesión**, Supabase dispara `PASSWORD_RECOVERY` con sesión, así que sin manejarlo el usuario volvía logueado y nadie le pedía la contraseña nueva. **Lo único que no se pudo probar sin mandar un correo real es el camino feliz**: queda para el QA. Diagnóstico completo en `docs/audits/2026-08-18-recuperar-contrasena.md`.
