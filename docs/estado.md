...
- [ ] **Recuperar contraseña: no existe el flujo.** Si una agencia olvida su contraseña hay que resolverlo a mano desde Supabase. Con MEGATOURS se puede; con diez agencias, no. **Investigado el 2026-08-18, con diagnóstico y plan en `docs/audits/2026-08-18-recuperar-contrasena.md`.** El hallazgo que decide el diseño: **el enlace del correo ES un inicio de sesión** (Supabase dispara `PASSWORD_RECOVERY` con sesión), así que sin manejarlo el usuario vuelve logueado y nadie le pide la contraseña nueva. Ya están el SMTP y la plantilla; falta el código, la ruta de retorno y las tres URLs en la lista blanca de Supabase.
