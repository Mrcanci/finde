...
- [ ] **Traducir las plantillas de correo de Supabase, que están en INGLÉS.** La invitación de prueba llegó diciendo "You have been invited to create a user on...". Los usuarios son peruanos y el resto del producto está en español. **Las cuatro plantillas ya están escritas y listas para pegar en `docs/plantillas-correo-supabase.md`** (confirmación, código de 6 dígitos, contraseña e invitación). Falta pegarlas a mano en **Authentication > Emails > Templates**, y **abrir los correos en Gmail, Outlook de Windows y Apple Mail**, que es lo único que no se pudo verificar. Conviene hacerlo **junto con reactivar la confirmación de correo**, que es cuando esos textos se empiezan a ver.
