...
- **Recuperar contraseña (2026-08-19)**: una agencia que olvidaba su contraseña no tenía ningún camino y había que resolverlo a mano desde Supabase. **El hallazgo que ordenó la tanda, y que quedó comentado en el código: el enlace del correo ES un inicio de sesión.** Supabase crea sesión y dispara `PASSWORD_RECOVERY`, así que sin manejarlo el usuario volvía logueado al inicio y nadie le pedía la contraseña nueva: habría sido peor que no tener el flujo, porque parece que funciona. El enlace "¿Olvidaste tu contraseña?" vive dentro de `AuthForm`, así que sale en el login y en el modal del checkout con un solo cambio, y es un **tercer modo del formulario y no una ruta**, porque en el modal navegar es romperlo. **Va con el borrador del checkout persistido**, que además resuelve la precondición del pendiente de Google. Diagnóstico y mediciones en `docs/audits/2026-08-18-recuperar-contrasena.md`: los dos comportamientos del enlace, vencido y válido, se midieron **sin mandar un solo correo**, pegándole al endpoint de verificación con un token inválido.
